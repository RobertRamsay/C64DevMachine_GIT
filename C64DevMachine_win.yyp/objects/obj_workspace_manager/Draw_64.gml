...
if (global.show_helper_window && instance_exists(global.helper_node)) {
    ...
    _desc = "Configures the VIC-II chip for the selected display mode. Sets bank, screen RAM, char/bitmap pointers, border and background colours. Supports TEXT, MCT, ECM, BITMAP and MCB modes - safe RMW on $DD00 preserves CIA serial bus bits.";
        ...
        _desc = "Four-way pixel-smooth camera over a META_TILESET room. The room is flattened at compile time into a char plane and a colour plane, so the runtime never decodes stamps. JSR Metascroll_L/R/U/D to move the camera one pixel that way, and JSR Metascroll_Update every frame so a coarse step still finishes when nothing is moving. Fine scroll is $D016 for L/R and $D011 for U/D; the visible window is rows 1-23, cols 1-38, with the outer row and column left blank to hide the edges. COLOUR MODE decides what happens to colour RAM. FIXED is the stock C64 setting: in multicolour text mode the three shared colours come from $D021/$D022/$D023 and the colour-RAM nibble only supplies the fourth, so a room drawn from one palette needs no colour scrolling at all - the nibble is written once at init, the coarse step costs a single frame and the colour plane is never built. SHIFT scrolls colour RAM per cell for maps that really do vary, splitting the coarse step over two frames - chars first, then colour - which is worth it on a C64 Ultimate in turbo but not on stock hardware.";
    } else if (_node.node_type == "MACRO_VSCROLL") {
