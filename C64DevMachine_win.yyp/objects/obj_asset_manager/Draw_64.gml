...
// ---- MAP SELECTOR ROW (wraps + scrolls; 4 visible rows) ----
  ...
  // Test area interaction (suppressed while the SLICE modal is open)
    ...
    if (_ts_chr_ref != noone) {
        ...
        if (_run_fx_on) {
            if (_clbhov && mouse_check_button_pressed(mb_left) && _run_mixed && !_ecm_mode) {
                ...
                while (array_length(_run_node.instructions[0]) < 8) { array_push(_run_node.instructions[0], 0); }
                _run_node.instructions[0][7] = _run_nib ^ 0x08;
                global.undo_dirty = true;
            }
        }
    ...
    for (var _pi2 = 0; _pi2 < 16; _pi2++) {
        ...
        if (_run_fx_on) {
            ...
            if (_pphov && !_locked && mouse_check_button_pressed(mb_left)) {
                ...
                _run_node.instructions[0][7] = _pal_nib;
    ...
    var _sa_x1  = _strip_x1 + 60;
    var _sa_x2  = _sa_x1 + 56;
