...
// Compact spine — close gap left by removed followers
            ...
            with (obj_c64_node) {
                ...
                // gap the fold leaves, so it kept claiming drags in empty space and
                // the node you were actually holding refused to move.
...
if (is_connected && prev_height != height && !global.drop_occurred_this_frame) {
    ...
    // Draw can derive a new height after a drop was finalized. Request a
    // follow-up only when that actually happens, not six frames after every drop.
