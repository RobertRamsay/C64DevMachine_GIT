/// @desc Draw body content for COND_IF node
function scr_node_draw_cond_if(_draw_x, _y) {
    ...
    draw_text_l((_var_bx1 + _var_bx2) / 2, _cy , _var == "" ? "< SELECT >" : scr_nloc_display_name(_var));
    draw_set_halign(fa_left);
    _cy += _line_h;

    // ── Row 1: == / != toggle  +  CMP value ─────────────────
    var _op_label = "==";
    switch (_mode) {
    ...
    // Left: literal value box (only drawn when no cmp var set)
