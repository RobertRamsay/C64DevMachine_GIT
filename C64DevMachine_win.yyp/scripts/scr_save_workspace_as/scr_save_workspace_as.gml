	function scr_save_workspace_as() {
	...
	if (instance_exists(obj_asset_manager)) {
        ...
        if (variable_struct_exists(_a.meta, "gradient_custom_count"))  _meta_out.gradient_custom_count  = _a.meta.gradient_custom_count;
	        // TONE-SORTED flag. The bytes on disk are already tone-sorted, so
	        // losing this would silently revert the picture to count-sort on the
	        // next save. See scr_c64_tone_group.
	        if (variable_struct_exists(_a.meta, "tone_sorted"))  _meta_out.tone_sorted  = _a.meta.tone_sorted;
	        // BITMAP collision tags — 1000 cells, 0 = untagged. Written only when
	        // at least one cell is tagged, so an untagged sheet costs nothing.
	...
	// CHAR_SET meta
        ...
        if (_a.type == "BITMAP_BUILDER") {
            ...
            // Collision tag-grid BYTE_DATA the builder owns (just the name — the
            // asset saves separately). tag_type is the last-used paint colour.
