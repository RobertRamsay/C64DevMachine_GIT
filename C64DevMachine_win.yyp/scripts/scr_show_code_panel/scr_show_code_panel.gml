...
///   1 = ASM    .0810            BEQ L_LEXIT_REF_INSTANCE_100064
///
...
function scr_show_code_bytes(_ln) {
    if (_ln.kind == "byte") {
        if (_ln.lbl != "" && _ln.hasres) {
            if (string_pos("_lab_hi", _ln.raw) > 0) {
                return scr_show_code_hex((_ln.res >> 8) & $FF, 2);
            }
            return scr_show_code_hex(_ln.res & $FF, 2);
        }
        if (_ln.lbl != "") {
            return "??";
        }
    ...
    if (_ln.lbl != "") {
        ...
        // arithmetic c64_new_program's "rel" fixup performs.
        if (scr_show_code_is_branch(_ln.mnem)) {
            var _off = _ln.res - (_ln.pc + 2);
            if (_off < 0) {
                _off = 256 + _off;
            }
            return _op + " " + scr_show_code_hex(_off & $FF, 2);
        }

        if (_ln.sz == 2) {
            if (string_pos("_lab_hi", _ln.raw) > 0) {
                return _op + " " + scr_show_code_hex((_ln.res >> 8) & $FF, 2);
            }
            return _op + " " + scr_show_code_hex(_ln.res & $FF, 2);
        }

        return _op + " " + scr_show_code_hex(_ln.res & $FF, 2)
                   + " " + scr_show_code_hex((_ln.res >> 8) & $FF, 2);
    }
...
function scr_show_code_text(_ln, _mode) {
    ...
    if (_ln.kind == "byte") {
        if (_ln.lbl != "" && _ln.hasres && _mode == 0) {
            if (string_pos("_lab_hi", _ln.raw) > 0) {
                return ".BYTE $" + scr_show_code_hex((_ln.res >> 8) & $FF, 2);
            }
            return ".BYTE $" + scr_show_code_hex(_ln.res & $FF, 2);
        }
        if (_ln.lbl != "") {
            if (string_pos("_lab_hi", _ln.raw) > 0) {
    ...
    if (_ln.lbl != "") {
        ...
        if (_mode == 0 && _ln.hasres) {
            if (string_pos("_lab_lo", _raw) > 0) {
                return _op3 + " #$" + scr_show_code_hex(_ln.res & $FF, 2);
            }
            if (string_pos("_lab_hi", _raw) > 0) {
                return _op3 + " #$" + scr_show_code_hex((_ln.res >> 8) & $FF, 2);
            }
    ...
    with (obj_workspace_manager) {
        ...
        for (var _i = 0; _i < array_length(_compiled); _i++) {
            ...
            if (_m == "org") {
                ...
                _pc  = _o;
                _run = -1;
                array_push(_flat, { kind:"org", key:"", name:"", owner:"", pc:_pc, raw:_m, mnem:_m, val:0, lbl:"", sz:0, res:0, hasres:false, count:0, vals:[], dkey:"" });
                continue;
            }
            ...
            if (array_length(_e) > 2) {
                ...
                if (is_string(_tag)) {
                    ...
                    if (instance_exists(_tag)) {
                        // _owner is set for every node type; _key only for
                        // macros, since only macros fold as macro groups.
            ...
            if (_m == "label") {
                ...
                _run = -1;
                // Hidden scaffolding labels are recorded above and dropped
                // here, which also stops them fragmenting a macro's run.
                if (ds_map_exists(_userlbl, _lbl)) {
                    array_push(_flat, { kind:"label", key:_key, name:_name, owner:_owner, pc:_pc, raw:_m, mnem:_m, val:0, lbl:_lbl, sz:0, res:0, hasres:false, count:0, vals:[], dkey:"" });
                }
                continue;
            }
            ...
            if (_m == "byte" || _m == "byt" || _m == "byte_lab_lo" || _m == "byte_lab_hi") {
                ...
                if (_lbl != "") {
                    _run = -1;
                    array_push(_flat, { kind:"byte", key:_key, name:_name, owner:_owner, pc:_pc, raw:_m, mnem:"byte", val:_v, lbl:_lbl, sz:1, res:0, hasres:false, count:1, vals:[], dkey:"" });
                    _pc += 1;
                    continue;
                }
                ...
                var _extend = false;
                if (_run >= 0) {
                    if (_flat[_run].key == _key && _flat[_run].owner == _owner) {
                ...
                if (_extend) {
                    ...
                    array_push(_flat, { kind:"data", key:_key, name:_dname, owner:_owner, pc:_pc, raw:"byte", mnem:"byte", val:0, lbl:"", sz:1, res:0, hasres:false, count:1, vals:[_v], dkey:"D:" + _owner + "@" + scr_show_code_hex(_pc, 4) });
            ...
            _run = -1;
            array_push(_flat, { kind:"op", key:_key, name:_name, owner:_owner, pc:_pc, raw:_m, mnem:_norm, val:_v, lbl:_lbl, sz:_sz, res:0, hasres:false, count:1, vals:[], dkey:"" });
            _pc += _sz;
        }

        // ---- PASS 2: resolve label operands now every label address is known.
        // Forward branches are the whole reason this is a second pass.
...
// =====================================================================
function scr_show_code_emit(_out, _flat, _idx, _indent) {
    var _ln = _flat[_idx];
...
function scr_show_code_fold() {
    ...
    with (obj_workspace_manager) {
        ...
        while (_i < _n) {
            ...
            if (_ln.key == "") {
                scr_show_code_emit(_out, showcode_flat, _i, 0);
                _i += 1;
                continue;
            }
            ...
            if (_open) {
                for (var _g = _i; _g < _j; _g++) {
                    scr_show_code_emit(_out, showcode_flat, _g, 10);
                }
            }
...
function scr_show_code_draw() {
    ...
    with (obj_workspace_manager) {
        ...
        var _pad_t = 6;
        var _pad_b = 12;
        var _grab  = 5;    // edge hit zone, GUI px

        showcode_w    = clamp(showcode_w, 300, 900);
        showcode_rows = clamp(showcode_rows, 5, 60);
        ...
        var _rows = clamp(showcode_rows, 1, max(1, min(60, _fit)));
        ...
        if (showcode_resize != 0) {
            if (mouse_check_button(mb_left)) {
                if (showcode_resize == 1) {
                    var _newx = clamp(_mx, 0, showcode_rs_edge - 300);
                    showcode_w = clamp(showcode_rs_edge - _newx, 300, 900);
                    showcode_x = showcode_rs_edge - showcode_w;
                } else if (showcode_resize == 2) {
                    showcode_w = clamp(_mx - _px, 300, 900);
                } else {
                    var _body_top = _py + _hdr_h + _pad_t;
                    showcode_rows = clamp(round((_my - _pad_b - _body_top) / _row_h), 5, 60);
                }

                _pw     = showcode_w;
                _px     = floor(showcode_x);
                _rows   = clamp(showcode_rows, 1, max(1, min(60, _fit)));
                _body_h = _rows * _row_h;
                _ph     = _hdr_h + _pad_t + _body_h + _pad_b;

                _btn_min_x = _px + _pw - 10 - _btn_w;
                _btn_mod_x = _btn_min_x - 6 - 52;
            } else {
                showcode_resize = 0;
                scr_show_code_save_ini();
            }
        ...
        global.showcode_mouse_over = (_over || showcode_dragging || showcode_resize != 0);
        ...
        for (var _r = 0; _r < _rows; _r++) {
            ...
            var _rhov   = (showcode_resize == 0 && !showcode_dragging &&
                           _mx >= _px + 6 && _mx < _px + _pw - 14 &&
            ...
            draw_text_transformed(_col_text + _indent, _ry, scr_show_code_text(_ln, showcode_mode), 1.0, 1.0, 0);
        }
