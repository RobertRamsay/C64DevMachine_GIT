...
case "MACRO_SCROLL": {
    ...
    // Use MACRO_MAP_SWITCH if present, otherwise MACRO_MAP — just needs to be connected
    with (obj_c64_node) {
    ...
    var _emit_scroll_loader = function(_lst, _lbl_entry, _dest_base, _colour_extra) {
        ...
        if (_map_w_is_word) {
            array_push(_lst, ["lda_lab", _lbl_scrollx,    _id]);
            array_push(_lst, ["sta_zp",  0xFD,            _id]);
            array_push(_lst, ["lda_lab", _lbl_scrollx_hi, _id]);
            array_push(_lst, ["sta_zp",  0xFE,            _id]);
        } else {
            array_push(_lst, ["ldy_lab", _lbl_scrollx, _id]);
        }
        array_push(_lst, ["ldx_imm", 0x00, _id]);
        array_push(_lst, ["label", _lbl_cols]);
        for (var _r = 0; _r < _row_count; _r++) {
            var _row_src = _map_base + _colour_extra + ((_r + _start_row) * _map_w);
            if (_map_w_is_word) {
                array_push(_lst, ["lda_imm", _row_src & 0xFF,        _id]);
                array_push(_lst, ["clc",     0,                      _id]);
                array_push(_lst, ["adc_zp",  0xFD,                   _id]);
                array_push(_lst, ["sta_zp",  0xF3,                   _id]);
                array_push(_lst, ["lda_imm", (_row_src >> 8) & 0xFF, _id]);
                array_push(_lst, ["adc_zp",  0xFE,                   _id]);
                array_push(_lst, ["sta_zp",  0xF4,                   _id]);
                array_push(_lst, ["ldy_imm", 0x00,                   _id]);
                array_push(_lst, ["lda_izy", 0xF3,                   _id]);
            } else {
                array_push(_lst, ["lda_aby", _row_src, _id]);
            }
            array_push(_lst, ["sta_abx", _dest_base + ((_r + _start_row) * 40), _id]);
        }
        if (_map_w_is_word) {
            array_push(_lst, ["lda_zp",  0xFD,                 _id]);
            array_push(_lst, ["clc",     0,                    _id]);
            array_push(_lst, ["adc_imm", 0x01,                 _id]);
            array_push(_lst, ["sta_zp",  0xFD,                 _id]);
            array_push(_lst, ["lda_zp",  0xFE,                 _id]);
            array_push(_lst, ["adc_imm", 0x00,                 _id]);
            array_push(_lst, ["sta_zp",  0xFE,                 _id]);
            array_push(_lst, ["lda_zp",  0xFE,                 _id]);
            array_push(_lst, ["cmp_imm", (_map_w >> 8) & 0xFF, _id]);
            array_push(_lst, ["bcc",     _lbl_nowrap,          _id]);
            array_push(_lst, ["bne",     _lbl_dowrap,          _id]);
            array_push(_lst, ["lda_zp",  0xFD,                 _id]);
            array_push(_lst, ["cmp_imm", _map_w & 0xFF,        _id]);
            array_push(_lst, ["bcc",     _lbl_nowrap,          _id]);
            array_push(_lst, ["label",   _lbl_dowrap]);
            array_push(_lst, ["lda_imm", 0x00,                 _id]);
            array_push(_lst, ["sta_zp",  0xFD,                 _id]);
            array_push(_lst, ["lda_imm", 0x00,                 _id]);
            array_push(_lst, ["sta_zp",  0xFE,                 _id]);
            array_push(_lst, ["label",   _lbl_nowrap]);
        } else {
            array_push(_lst, ["iny",     0,             _id]);
            array_push(_lst, ["cpy_imm", _map_w & 0xFF, _id]);
            array_push(_lst, ["bcc",     _lbl_nowrap,   _id]);
            array_push(_lst, ["ldy_imm", 0x00,          _id]);
            array_push(_lst, ["label",   _lbl_nowrap]);
        }
        array_push(_lst, ["inx",     0,           _id]);
        array_push(_lst, ["cpx_imm", _cam_w,      _id]);
        array_push(_lst, ["beq",     _lbl_done,   _id]);
        array_push(_lst, ["jmp_abs", _lbl_cols,   _id]);
        array_push(_lst, ["label",   _lbl_done]);
        array_push(_lst, ["rts",     0,           _id]);
    };
