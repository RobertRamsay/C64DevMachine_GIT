function scr_node_draw_macro_load_game(_draw_x, _y, _cam_x, _cam_y, _cam_zoom) {
    var _header_h = 24;
    var _line_h   = 12;
    ...
    var _c_edit = make_color_rgb(220, 210, 60);
    ...
    if (_org_asset != undefined && _file_name != "") {
        ...
        if (!_is_linked) {
            ...
            draw_set_halign(fa_center);
            draw_set_color(make_color_rgb(220, 210, 60));
            draw_text_l(_draw_x + (width / 2), _ly, "READS FROM DISK");
            draw_set_halign(fa_left);
        }
    }
}